...
public class Main {
    public static void main(String[] args) {
        Actions[] players = {new Human(2000.3, 1.0),
                new Cat(1500.00, 0.5),
                new Robot(8000.5, 4.3)};


        IBarrier[] barriers = {new Treadmill(1000), new Wall(0.6), new Treadmill(2000), new Wall(3)};


        for (Actions p : players) {
            for (IBarrier b : barriers) {
                if (p.isActivePlayer()) {
                    if (b instanceof IWall) {
                        p.jump((IWall) b);
                    } else {
                        p.run((ITreadmill) b);
                    }
